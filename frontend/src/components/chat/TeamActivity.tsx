...
import { agentStatusMeta, roleLabelKey } from "../../lib/labels";
...
function EventRow({ event, t }: { event: TeamEvent; t: TFunction }) {
  const key = roleLabelKey(event.agent);
  const agent = key !== null ? t(key) : event.agent;
...
export function TeamActivity({ events }: { events: TeamEvent[] }) {
  ...
  return (
    ...
    >
      ...
      {visible.length === 0 ? (
        <p className="px-4 py-6 text-xs text-neutral">{t("chat.activity.empty")}</p>
      ) : (
